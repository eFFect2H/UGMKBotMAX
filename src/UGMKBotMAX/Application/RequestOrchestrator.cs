...
public sealed class RequestOrchestrator
{
    ...
    public async Task<Guid> CreateRequestAsync(NewRequestInput input, CancellationToken cancellationToken)
    {
        ...
        var targetChatId = _router.ResolveTargetChatId(request);
        await _bot.SendRequestToChatAsync(targetChatId, request, cancellationToken);
        await _bot.NotifyApplicantRequestAcceptedAsync(request, cancellationToken);

        _logger.LogInformation("Created request {RequestId} and routed to {TargetChatId}", request.Id, targetChatId);
