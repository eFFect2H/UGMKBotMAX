...
public sealed class BotWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        await _gateway.RunAsync(
            onNewRequest: async (input, ct) => await _orchestrator.CreateRequestAsync(input, ct),
            onCloseRequest: async (requestId, chatId, ct) => await _orchestrator.CloseRequestAsync(requestId, chatId, ct),
            onReportRequested: async (_, ct) => await _orchestrator.GetOpenRequestsAsync(ct),
